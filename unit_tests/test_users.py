...
class UsersTest(JenkinsTest):
    ...
    def test_get_admin_data(self):
        ...
        orig_password = config["password"]
        try:
            # Ensure our password is empty so it's autogenerated.
            config["password"] = ""
            ...
            # We're setting the password to a specific value.
            self.useFixture(MonkeyPatch("charmhelpers.core.host.pwgen", pwgen))
            self.users.configure_admin()
            # Having configured the admin user, let's query _admin_data to
            # confirm we read the password correctly from disk.
            self.assertThat(paths.ADMIN_PASSWORD, FileContains("x"))
            admin = self.users._admin_data()
            self.assertEqual(admin.password, "x")
        finally:
            config["password"] = orig_password
